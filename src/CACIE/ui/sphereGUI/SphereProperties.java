...
public class SphereProperties
{
  ...
  private static final int NUM_SPHERES = 16;

  private static final String PROP_FNM = "./ConfigData/SphereProps.txt";
  ...
  {
	    String prop;
		if ((prop = life3DProps.getProperty("num")) != null)
	      return checkNumSpheres(prop);
	    else {
	      System.out.println("No width property found; using " + NUM_SPHERES);
	      return NUM_SPHERES;
	    }
	  
  }
  
  private int checkNumSpheres(String numStr)
  /* the width string must be an integer between MIN_LEN and the 
     screen's width */
  {
    int num = extractInt(numStr);
//    if (num < MIN_LEN) {
//      System.out.println("width too small; set to " + MIN_LEN);
//      return MIN_LEN;
//    }
//    else if (num > screenDim.width) {
